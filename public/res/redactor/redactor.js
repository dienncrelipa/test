...
(function($)
{
	...
	$.Redactor.opts = {
		...
		pasteInlineTags: ['br', 'strong', 'ins', 'code', 'del', 'samp', 'kbd', 'sup', 'sub', 'mark', 'var', 'cite', 'small', 'b', 'u', 'em', 'i', 'quote', 'red', 'yellow'],
		...
		buttons: ['format', 'bold', 'italic', 'deleted', 'link', 'lists', 'image', 'file',  'horizontalrule', 'red'], // + 'underline', 'ol', 'ul', 'indent', 'outdent'
		...
		shortcuts: {
			...
			'ctrl+shift+l, meta+shift+l': { func: 'inline.format', params: ['subscript'] },
		...
		langs: {
			en: {
				...
				"indent": "インデントを増やす",
				"horizontalrule": "横ライン",
				"upload-label": "アップロードするファイルをドロップ",
				"caption": "キャプション",
		...
		buffer: [],
		rebuffer: [],
		inlineTags: ['a', 'span', 'strong', 'strike', 'b', 'u', 'em', 'i', 'code', 'del', 'ins', 'samp', 'kbd', 'sup', 'sub', 'mark', 'var', 'cite', 'small', 'quote', 'red', 'yellow'],
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		button: function()
		{
			return {
				...
				build: function(btnName, btnObject)
				{
					...
					// Custom 8.11.2.16
					var titleCustom = createShortcut.createTitleForShortCuts(btnName, btnObject.title);
					var $button = $('<a href="javascript:void(null);" class="re-button re-' + btnName + '" title="' + titleCustom + '" rel="' + btnName + '" />').html(btnObject.title);
		...
		image: function()
		{
			return {
				...
				{
					var $img = $('<img>');
					var url = $('input[name="image-insert-url"]').val();
					var $figure = $('<figure>');
					$img.attr('src', url);
					...
					var source = $('input[name="image-source-url"]').val();
					if(source.length > 0) {
						this.caret.after($figure);
						var sourceText = ($('input[name="image-source-text"]').val().length > 0)
							? $('input[name="image-source-text"]').val() : source;
						var sourceNode = $('<p class="quote-origin-redactor" style="font-size: 11px; "/>').html('Source: <a href="'+source+'">'+sourceText+'</a>');
						this.insert.node(sourceNode);
					}
				},
				show: function()
				{
					...
					this.modal.getActionButton().click(this.image.urlInsert);
					this.modal.show();
				},
				insert: function(json, direct, e)
				{
				...
				showEdit: function($image)
				{
					...
					if($link.hasClass('button18')) {
						this.link.show($link, true);
						return;
					}
		...
		inline: function()
		{
			return {
				...
				formatSet: function(tag, attr, value, type)
				{
					...
					if(prevNode.prop('tagName') == 'P' && prevNode.html() == '\u200B') {
						prevNode.remove();
					}
					if(nextNode.prop('tagName') === 'BR') {
						nextNode.remove();
					}
					this.caret.start(node);
				},
		...
		keydown: function()
		{
			return {
				init: function(e)
				{
					...
					this.keydown.range = this.selection.range(this.selection.get());
					...
					this.keydown.div = this.utils.isTag(this.keydown.current, 'div');
					// callback
					...
					// backspace & delete
					theBackspaceDeleteHandle: if (key === this.keyCode.BACKSPACE || key === this.keyCode.DELETE)
					{
						var $block = $(this.keydown.block);
						var $prevBlock = $block.prev();
						var $nextBlock = $block.next();

						var exceptClass = ['div.reviewbox', 'div.checkbox', 'div.qa', 'div.conv', 'h3.rank'];
						var queryString = exceptClass.join(', ');
						if(!$prevBlock.is(queryString) && !$nextBlock.is(queryString)) {
							break theBackspaceDeleteHandle;
						}

						var blocks = this.block.getBlocks();

						var isSelectionContainException = false;
						$.each(blocks, function(k, e){
							if($(e).is(queryString)) {
								isSelectionContainException = true;
								return false;
							}
						});

						if(isSelectionContainException) {
							e.preventDefault();
							return;
						}

						var selection = this.keydown.range;
						var isBeginOfLine = this.utils.isStartOfElement(selection.startContainer);
						var isEndOfLine = this.utils.isEndOfElement(selection.startContainer);

						if( $prevBlock.is(queryString) && $block.text() == ''){
							$block.remove();
							e.preventDefault();
							return;
						}
						if( ($prevBlock.get(0) == undefined || $prevBlock.is(queryString))
								&& key === this.keyCode.BACKSPACE && ($block.text() == '' || isBeginOfLine) ) {
							e.preventDefault();
							return;
						}
						if( ($nextBlock.get(0) == undefined || $nextBlock.is(queryString))
								&& key === this.keyCode.DELETE && ($block.text() == '' || isEndOfLine) ) {
							e.preventDefault();
							return;
						}
				...
				onBackspaceAndDeleteAfter: function(e)
				{
					...
					setTimeout($.proxy(function()
					{
						...
						this.core.editor().find('*[style]').not('img, #redactor-image-box, #redactor-image-editter, ' +
							'mark, p.quote-origin-redactor, iframe.instagram-media, div.insta-slim, iframe.twitter-iframe, iframe.pdb-product-iframe, table, td, tr, th, tbody, thead').removeAttr('style');
				...
				{
					var k = this.keyCode;
					var keys = [k.BACKSPACE, k.DELETE, k.ENTER, k.ESC, k.TAB, k.CTRL, k.META, k.ALT, k.SHIFT, 229];
				...
				{
					var tags = [this.keydown.blockquote, this.keydown.pre, this.keydown.figcaption, this.keydown.div];
				...
				insertAfterLastElement: function(element)
				{
					...
					if ($next.length !== 0 && $next.tagName == 'DIV' && element.tagName == 'DIV')
					{
						var node = $(this.opts.emptyHtml);
					...
					else
					{
						...
						{
							var parent = $(element).parents("div.qa, div.checkbox, div.ranking");
							if(!$(element).is(".qa, .ranking, .checkbox") && parent.length > 0)
							{
								element = parent[0];
							}
						}
			...
			return {
				...
				unlink: function(e)
				{
					...
					this.core.callback('deletedLink', $links);
                                ...
                                updateIDLink: function($el){
                                    if(typeof $el == 'undefined'){
                                        return;
                                    }
                                    $el.addClass('btn-index');
                                },
				insert: function(link, cleaned)
				{
					...
					if ($el === false)
					{
						...
						switch (link.bkind) {
							case 'link':
								$el.removeClass().addClass('link');
								break;
							case 'btn btn-default':
								$el.removeClass().addClass('btn btn-default');
								break;
							case 'jelly':
								$el.removeClass().addClass('btn-jelly');
								break;
							case 'button01':
								$el.removeClass().addClass('button button01');
								break;
							case 'button02':
								$el.removeClass().addClass('button button02');
								break;
							case 'button03':
								$el.removeClass().addClass('button button03');
								break;
							case 'button04':
								$el.removeClass().addClass('button button04');
								break;
							case 'button05':
								$el.removeClass().addClass('button button05');
								break;
							case 'button06':
								$el.removeClass().addClass('button button06');
								break;
							case 'button07':
								$el.removeClass().addClass('button button07');
								break;
							case 'button08':
								$el.removeClass().addClass('button button08');
								break;
							case 'button09':
								$el.removeClass().wrap('<p></p>').wrap('<div class="button09"></div>');
								break;
							case 'button10':
								$el.removeClass().wrap('<p></p>').wrap('<div class="button10"></div>');
								break;
							case 'button11':
								$el.removeClass().addClass('button11');
								break;
							case 'button12':
								$el.removeClass().addClass('button12');
								break;
							case 'button13':
								$el.removeClass().addClass('button13');
								break;
							case 'button14':
								$el.removeClass().addClass('button12 button14 animated bounceIn infinite');
								break;
							case 'button15':
								$el.removeClass().addClass('button13 button15 animated bounce infinite');
								break;
							case 'button16':
								$el.removeClass().addClass('button12 button16 animated rubberBand infinite');
								break;
							case 'button17':
								$el.removeClass().addClass('button13 button17 animated shake infinite');
								break;
							case 'button18':
								$el.removeClass().addClass('button18').html('<img src="//s3image.com/gSWEaZK9J4DZvUYV.gif" />');
								break;
							case 'no-style':
								$el.removeClass().addClass('link');
								break;
				...
				updateElementLinkType: function(el, parent, klass) { 
					if(typeof parent[0] !== 'undefined') {
						if(parent[0].nodeName.toLowerCase() == 'div') {
							el.unwrap(parent);
							if(typeof el.parent('div')[0] !== 'undefined' && $(el.parent('div')[0]).hasClass('redactor-editor') === false) {
								el.unwrap('div');
							}
						}
					}
					
					if(klass != 'button09' || klass != 'button10') {
						if(typeof el.parent('p')[0] === 'undefined') {
							el.removeClass().addClass(klass).wrap('<p></p>');
						} else {
				...
				update: function($el, link)
				{
					...
					switch (link.bkind) {
						case "link":
							this.link.updateElementLinkType($el, $el.parent(0), "link");
							break;

						case "btn btn-default":
							this.link.updateElementLinkType($el, $el.parent(0), "btn btn-default");
							break;

						case "jelly":
							this.link.updateElementLinkType($el, $el.parent(0), "btn-jelly");
							break;

						case "button01":
							this.link.updateElementLinkType($el, $el.parent(0), "button button01");
							break;

						case "button02":
							this.link.updateElementLinkType($el, $el.parent(0), "button button02");
							break;

						case "button03":
							this.link.updateElementLinkType($el, $el.parent(0), "button button03");
							break;

						case "button04":
							this.link.updateElementLinkType($el, $el.parent(0), "button button04");
							break;

						case "button05":
							this.link.updateElementLinkType($el, $el.parent(0), "button button05");
							break;

						case "button06":
							this.link.updateElementLinkType($el, $el.parent(0), "button button06");
							break;

						case "button07":
							this.link.updateElementLinkType($el, $el.parent(0), "button button07");
							break;

						case "button08":
							this.link.updateElementLinkType($el, $el.parent(0), "button button08");
							break;

						case "button09":
							this.link.updateElementLinkType($el, $el.parent(0), "button09");
							$el.removeClass().wrap('<div class="button09"></div>');
							break;

						case "button10":
							this.link.updateElementLinkType($el, $el.parent(0), "button10");
							$el.removeClass().wrap('<div class="button10"></div>');
							break;

						case "button11":
							this.link.updateElementLinkType($el, $el.parent(0), "button11");
							break;

						case "button12":
							this.link.updateElementLinkType($el, $el.parent(0), "button12");
							break;

						case "button13":
							this.link.updateElementLinkType($el, $el.parent(0), "button13");
							break;

						case "button14":
							this.link.updateElementLinkType($el, $el.parent(0), "button12 button14 animated bounceIn infinite");
							break;

						case "button15":
							this.link.updateElementLinkType($el, $el.parent(0), "button13 button15 animated bounce infinite");
							break;

						case "button16":
							this.link.updateElementLinkType($el, $el.parent(0), "button12 button16 animated rubberBand infinite");
							break;

						case "button17":
							this.link.updateElementLinkType($el, $el.parent(0), "button13 button17 animated shake infinite");
							break;

						case "button18":
							this.link.updateElementLinkType($el, $el.parent(0), "button18");
							$el.html('<img src="//s3image.com/gSWEaZK9J4DZvUYV.gif" />');
							break;
				...
				show: function(e, isForce)
				{
					var isForceLink = isForce || false;
					var tempNode = $('<p class="tempNode" style="margin: 0; line-height: 0;" />').html("&#8203;");
					this.core.editor().prepend(tempNode);
					// if call from clickable element
					if (typeof e !== 'undefined' && e.preventDefault)
					{
						e.preventDefault();
					...
					// build modal
					this.link.buildModal($el, isForceLink);
					...
					// delete temp node
					$(this.core.textarea()).on('modalClosed.callback.redactor', function(){
						$('p.tempNode').remove()
					});
				...
				setModalValues: function(link)
				{
					...
					if(link.bkind != '') {
						$('#redactor-link-type option').each(function () {
							if(typeof link.bkind !== 'undefined') {
								if(link.bkind.indexOf($(this).val()) != -1) {
									$('#redactor-link-type option').removeAttr('selected').filter('[value="' + $(this).val() + '"]').attr('selected', true);
									$('.link-kinds-list li').removeClass('active');
									$('.link-kinds-list li[data="'+$(this).val()+'"]').addClass('active');
								}
							}
						});
					}
					...
					$('.type-item').on('click', function() {
						...
						$('#redactor-link-type option').removeAttr('selected');
						$('#redactor-link-type option[value="'+data+'"]').attr('selected', 'selected');
					});

					$('#redactor-link-url').val(link.url);
					$('#redactor-link-url-text').val(link.text);
					$('#redactor-link-jelly-button').on('click', function() {
						$(this).is(":checked") == true ? $('#redactor-link-button').attr('disabled', true) : $('#redactor-link-button').attr('disabled', false);
					});

					$('#redactor-link-button').on('click', function() {
						$(this).is(":checked") == true ? $('#redactor-link-jelly-button').attr('disabled', true) : $('#redactor-link-jelly-button').attr('disabled', false);
					});
				},
				buildModal: function($el, isForceLink)
				{
					...
					if(isForceLink) {
						$btn.prop('forceLink', true);
						this.link.forceLink = $el;
					}
				},
				callback: function()
				{
					...
					if(this.modal.getActionButton().prop('forceLink') == true) {
						link.force = true;
					}
				...
				buildLinkFromModal: function()
				{
					...
					link.jelly= ($('#redactor-link-jelly-button').prop('checked')) ? true : false;
					link.bkind = $('#redactor-link-type option:selected').val();
				...
				buildLinkFromElement: function($el)
				{
					...
					if ($el !== false)
					{
						...
						link.jelly = this.link.buildJellyButton($el);
						link.bkind = typeof $el.attr('class') !== 'undefined' ? $el.attr('class') : $el.parent('div').attr('class');
					}
				...
				buildJellyButton: function($el) {
					return $el.hasClass('btn-jelly');
				},
				buildButton: function($el)
				{
					return $el.hasClass('btn btn-default');
				},
			...
			return {
				...
				convertLinks: function(html)
				{
					...
					for (var i = 0; i < length; i++)
					{
						...
						html = html.replace(regexp, '<a href="' + linkProtocol + $.trim(href) + '" class="btn-index redactor-linkify-object">' + $.trim(text) + '</a>');
					}
		...
		list: function()
		{
			return {
				toggle: function(cmd)
				{
					...
					if (this.utils.isRedactorParent($listParent) && $listParent[0].tagName !== 'LI' && this.utils.isBlock($listParent) && $.inArray($listParent[0].tagName, ['TH', 'TD']) ==  -1)
					{
						this.selection.save();
		...
		modal: function()
		{
			return {
				...
				templates: function()
				{
					this.opts.modal = {
						...
						+ '<div class="redactor-modal-tab redactor-group" data-title="General">'
							...
							+ '<div class="redactor-modal-tab-area">'
								...
								+ '<section>'
									...
									+ '<button id="redactor-modal-button-delete" class="redactor-modal-button-offset">削除</button>'
								+ '</section>'
							+ '</div>'
						+ '</div>',
						...
						+ '<div class="redactor-modal-tab" data-title="Upload">'
							...
							+ '<section>'
								+ '<label>画像URL</label>'
								+ '<input type="text" name="image-insert-url" />'
							+ '</section>'
							+ '<section>'
								+ '<label>画像サイトURL</label>'
								+ '<input type="text" name="image-source-url" />'
							+ '</section>'
							+ '<section>'
								+ '<label>画像ソーステキスト</label>'
								+ '<input type="text" name="image-source-text" />'
							+ '</section>'
							+ '<section>'
							+ '<button id="redactor-modal-button-action">挿入</button>'
							+ '<button id="redactor-modal-button-cancel">キャンセル</button>'
						...
						+ '<div class="redactor-modal-tab" data-title="General">'
							...
							+ '</section>'
							+ '<section>'
								+ '<label class="lbl-option-new-tab checkbox"><input type="checkbox" id="redactor-link-blank"> ' + this.lang.get('link-in-new-tab') + '</label>'
							+ '</section>'
							+ '<section>'
								+ '<div id="link-kinds-selection">'
									+ '<ul class="link-kinds-list">'
										+ '<li class="type-item" data="link"><span class="item-preview"><a href="javascript:void(0)" class="link">リンク</a></span></li>'
										+ '<li class="type-item" data="btn btn-default"><span class="item-preview"><a href="javascript:void(0)" class="btn btn-default">ボタン</a></span></li>'
										+ '<li class="type-item" data="jelly"><span class="item-preview"><a href="javascript:void(0)" class="btn-jelly">Jelly</a></span></li>'
										+ '<li class="type-item" data="button01"><p class="item-preview"><a href="javascript:void(0)" class="button button01">タイプ 01</a></p></li>'
										+ '<li class="type-item" data="button02"><p class="item-preview"><a href="javascript:void(0)" class="button button02">タイプ 02</a></p></li>'
										+ '<li class="type-item" data="button03"><p class="item-preview"><a href="javascript:void(0)" class="button button03">タイプ 03 </a></p></li>'
										+ '<li class="type-item" data="button04"><p class="item-preview"><a href="javascript:void(0)" class="button button04">タイプ 04</a></p></li>'
										+ '<li class="type-item" data="button05"><p class="item-preview"><a href="javascript:void(0)" class="button button05">タイプ 05</a></p></li>'
										+ '<li class="type-item" data="button06"><p class="item-preview"><a href="javascript:void(0)" class="button button06">タイプ 06</a></p></li>'
										+ '<li class="type-item" data="button07"><p class="item-preview"><a href="javascript:void(0)" class="button button07">タイプ 07</a></p></li>'
										+ '<li class="type-item" data="button08"><p class="item-preview"><a href="javascript:void(0)" class="button button08">タイプ 08</a></p></li>'
										+ '<li class="type-item" data="button09"><p class="item-preview"><div class="button09"><a href="javascript:void(0)" class="">タイプ 09</a></div></p></li>'
										+ '<li class="type-item" data="button10"><p class="item-preview"><div class="button10"><a href="javascript:void(0)" class="">タイプ 10</a></div></p></li>'
										+ '<li class="type-item" data="button11"><p class="item-preview"><a href="javascript:void(0)" class="button11">タイプ 11</a></p></li>'
										+ '<li class="type-item" data="button12"><p class="item-preview"><a href="javascript:void(0)" class="button12">タイプ 12</a></p></li>'
										+ '<li class="type-item" data="button13"><p class="item-preview"><a href="javascript:void(0)" class="button13">タイプ 13</a></p></li>'
										+ '<li class="type-item" data="button14"><p class="item-preview"><a href="javascript:void(0)" class="button12 button14 animated bounceIn infinite">タイプ 14</a></p></li>'
										+ '<li class="type-item" data="button15"><p class="item-preview"><a href="javascript:void(0)" class="button13 button15 animated bounce infinite">タイプ 15</a></p></li>'
										+ '<li class="type-item" data="button16"><p class="item-preview"><a href="javascript:void(0)" class="button12 button16 animated rubberBand infinite">タイプ 16</a></p></li>'
										+ '<li class="type-item" data="button17"><p class="item-preview"><a href="javascript:void(0)" class="button13 button17 animated shake infinite">タイプ 17</a></p></li>'
										+ '<li class="type-item" data="button18"><p class="item-preview"><a href="javascript:void(0)" class="button18"><img src="//s3image.com/gSWEaZK9J4DZvUYV.gif" style="width: 200px;" /></a></p></li>'
									+ '</ul>'
								+ '</div>'
								+ '<select id="redactor-link-type">'
										+ '<option value="no-style" selected>タイプ を選んでください</option>'
										+ '<option value="link">リンク</option>'
										+ '<option value="btn btn-default">ボタン</option>'
										+ '<option value="jelly">Jelly</option>'
										+ '<option value="button01" >タイプ 1</option>'
										+ '<option value="button02" >タイプ 2</option>'
										+ '<option value="button03" >タイプ 3</option>'
										+ '<option value="button04" >タイプ 4</option>'
										+ '<option value="button05" >タイプ 5</option>'
										+ '<option value="button06" >タイプ 6</option>'
										+ '<option value="button07" >タイプ 7</option>'
										+ '<option value="button08" >タイプ 8</option>'
										+ '<option value="button09" >タイプ 9</option>'
										+ '<option value="button10" >タイプ 10</option>'
										+ '<option value="button11" >タイプ 11</option>'
										+ '<option value="button12" >タイプ 12</option>'
										+ '<option value="button13" >タイプ 13</option>'
										+ '<option value="button14" >タイプ 14</option>'
										+ '<option value="button15" >タイプ 15</option>'
										+ '<option value="button16" >タイプ 16</option>'
										+ '<option value="button17" >タイプ 17</option>'
										+ '<option value="button18" >タイプ 18</option>'
								+ '</select>'
							+ '</section>'
							+ '<section>'
								+ '<button id="redactor-modal-button-action">挿入</button>'
				...
				show: function()
				{
					...
					$(document.activeElement).blur();
					this.$modalBox.find('input, textarea').eq(0).focus();
					...
					if (href.length > 24)
					{
						href = href.substring(0, 24) + '...';
                                        ...
                                        if($link.attr('href') != 'javascript:void(0)'){
                                            aLink = $('<a href="' + $link.attr('href') + '" target="_blank" />').html(href).addClass('redactor-link-tooltip-action');
                                            tooltip.append(aLink).append(' | ');
                                        }
					var aEdit = $('<a href="#" />').html(this.lang.get('edit')).on('click', $.proxy(this.link.show, this)).addClass('redactor-link-tooltip-action');
					var aUnlink = $('<a href="#" />').html(this.lang.get('unlink')).on('click', $.proxy(this.link.unlink, this)).addClass('redactor-link-tooltip-action');

					tooltip.append(aEdit).append(' | ').append(aUnlink);
		...
		toolbar: function()
		{
			return {
				...
				observeScrollEnable: function(scrollTop, boxTop)
				{
					...
					var end = boxTop + this.core.box().outerHeight(true) - 32;
		...
		utils: function()
		{
			return {
				...
				breakBlockTag: function()
				{
					...
					var isExceptTag = ['h2', 'h3', 'h4'].indexOf(tag) > -1;

					if ((!isEmpty || isExceptTag) && this.utils.isEndOfElement(block))
					{
						return { $block: $(block), $next: $(block).next(), type: 'end' };
					}
					else if ((!isEmpty || isExceptTag) && this.utils.isStartOfElement(block))
					{

...
@section('mainContent')
    ...
    <script>
        ...
        $(function(){
            ...
            $(document).on('click', '.img-thumb img', function(){
                $('.img-thumb img').removeClass('image-choosen');
                $(this).addClass('image-choosen');
                window.opener.thumbInsert($(this).attr('source-url'));
            });
        });
    </script>
    <div class="row" style="padding: 10px;">
        <div class="form-group form-inline" style="position: fixed; top: 0; width: 100%; background-color: #FFF; padding: 10px; z-index: 99999;">
            @if(!($currentKeyword = $currentRequest->get('keyword', false)))
            ...
            @else
            <script>loadImage('{{ $currentKeyword }}', currentPage, function(){})</script>
            @endif
